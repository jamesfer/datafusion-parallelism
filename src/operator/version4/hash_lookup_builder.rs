...
impl LocalAccumulator {
    ...
    pub fn submit(self) -> SharedCompactor {
        self.shared_compactor_constructor.build(self.state, self.local_shard_contents, self.batch_list)
    }
}
...
struct SharedCompactorConstructor {
    ...
    shared_shard_contents: Vec<LimitedRc<Vec<CachePadded<AtomicCell<Vec<LocalShardEntry>>>>>>,
...
impl SharedCompactorConstructor {
    ...
    pub fn build(self, state: LimitedRc<HashMapState>, local_shard_contents: Vec<Vec<LocalShardEntry>>, batch_list: LimitedRc<boxcar::Vec<(usize, RecordBatch)>>) -> SharedCompactor {
...
struct HashMapState {
    hash_lookup: DashMap<u64, (usize, usize)>,
    overflow_buffers: Vec<CachePadded<AtomicCell<Vec<(usize, usize)>>>>,
}
...
impl SharedCompactor {
    ...
    pub async fn compact(self, schema: SchemaRef) -> Result<(RecordBatch, impl IndexLookup<u64>), DataFusionError> {
        ...
        while let Ok((shard_number, all_shard_contents)) = self.shards_to_compact_receiver.recv_async().await {
            let shard_buffer = Self::compact_shard(shard_number, all_shard_contents, &self.state.hash_lookup);
        ...
        let join_map_result = self.build_join_map_when_last.run(self.state, |state| {
            let buffers = state.overflow_buffers.into_iter()
                .map(|cell| cell.take())
                .collect::<Vec<_>>();
            Arc::new(ReadOnlyJoinMap::new(state.hash_lookup, buffers))
        });
    ...
    fn publish_local_shard_contents(
        shared_shard_contents: Vec<LimitedRc<Vec<CachePadded<AtomicCell<Vec<LocalShardEntry>>>>>>,
        local_shard_contents: Vec<Vec<LocalShardEntry>>,
        instance_number: usize,
        shards_to_compact_sender: flume::Sender<(usize, Vec<CachePadded<AtomicCell<Vec<LocalShardEntry>>>>)>,
    ...
    fn compact_shard(
        ...
        output_hash_lookup: &DashMap<u64, (usize, usize)>,
    ) -> Vec<(usize, usize)> {
...
pub struct ReadOnlyJoinMap {
    shift: usize,
    hasher: RandomState,
    hash_lookup: ReadOnlyView<u64, (usize, usize)>,
    overflow_buffers: Vec<Vec<(usize, usize)>>,
}
...
impl ReadOnlyJoinMap {
    pub fn new(
        ...
        let shard_amount = hash_lookup.shards().len();
        let shift = mem::size_of::<usize>() * 8 - shard_amount.trailing_zeros() as usize;
        Self {
            shift,
            hasher: hash_lookup.hasher().clone(),
...
impl IndexLookup<u64> for ReadOnlyJoinMap {
    ...
    fn get_iter<'a>(&'a self, hash: &'a u64) -> Self::It<'a> {
        let maybe_entry = self.hash_lookup.get(hash)
            .map(|value| {
                let (stored_index, global_index) = value;
                (*stored_index, *global_index)
            });

        match maybe_entry {
            Some((stored_index, global_index)) => {
                let internal_hash = self.hasher.hash_one(hash) as usize;
                let shard_number = self.determine_shard(internal_hash);
                ReadOnlyJoinMapIterator::new(stored_index, global_index, &self.overflow_buffers[shard_number])
            },
            None => {
                ReadOnlyJoinMapIterator::empty()
            },
        }
    }
}
...
const EMPTY_BUFFER: [(usize, usize); 1] = [(0usize, 0usize)];
...
impl <'a> ReadOnlyJoinMapIterator<'a> {
    fn new(stored_index: usize, global_index: usize, overflow: &'a [(usize, usize)]) -> Self {
        Self {
            stored_index,
            global_index,
            overflow
...
impl Iterator for ReadOnlyJoinMapIterator<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.stored_index == 0 {
            return None;
        }

        let output = self.global_index;
        (self.stored_index, self.global_index) = self.overflow[self.stored_index];
        Some(output)
    }
}
...
pub fn create_local_accumulators(count: usize) -> Vec<LocalAccumulator> {
    ...
    let shared_shard_contents = transpose(
        (0..shard_count)
            .map(|_| {
                LimitedRc::new_copies(
    ...
    let state = HashMapState {
        ...
        overflow_buffers: (0..shard_count)
            .map(|_| CachePadded::new(AtomicCell::new(vec![])))
            .collect(),
    };
...
mod tests {
    ...
    use crate::operator::version4::hash_lookup_builder::create_local_accumulators;
    use crate::utils::index_lookup::IndexLookup;
